...
```{r}
# +geom_bar() adds bars to the graph (number of hikes per rating)
ggplot(hikes, aes(x = rating)) +
  geom_bar()
```

```{r}
# + labs(x = "Rating", y = "Number of hikes") adds names to x and y axis
ggplot(hikes, aes(x = rating)) +
  geom_bar() +
...
```{r}
# geom_bar(fill = "blue") fills bars in color
ggplot(hikes, aes(x = rating)) +
  geom_bar(fill = "blue") +
  labs(x = "Rating", y = "Number of hikes")
```
...
```{r}
# geom_bar(color = "orange",outlines bars in color
ggplot(hikes, aes(x = rating)) +
...
```{r}
# + theme_minimal() makes background white
ggplot(hikes, aes(x = rating)) +
  geom_bar(color = "orange", fill = "blue")  +
  labs(x = "Rating", y = "Number of hikes") +
  theme_minimal()
...
to add visual elements to the code

-   We added the bars using `geom_bar()`? Why "geom"?
because it's a geometric element.

-   What does `labs()` stand for?
labels, used for labeling axes.

-   What's the difference between `color` and `fill`?
color defines the outline, fill defines the rest of the geom.
...
summary: 27 hikes in the Adirondacks were rated moderate, 12 were rated easy, and 6 were rated difficult.

#### Part c {.unnumbered}

Is there anything you don't like about this barplot? For example: check out the x-axis again.

I don't like the order of the difficulty ratings on the x-axis. It should go in order from easiest to most difficult.
...
```
There are too many different elevations for this to be a helpful visualization of elevations.
...
A typical elevation for a hike in the Adirondacks is between 4000 and 4900 ft. The elevation range with the maximum amount of hikes is 3900 to 4200 ft in elevation. The elevation range with the minimum amount of hikes is 5100 to 5500 ft. The values are left-skewed. The elevation outliers under 3900 ft and over 5100 ft.
...
```{r}
# + geom_histogram() turns the bar graph into a histogram
ggplot(hikes, aes(x = elevation)) +
  geom_histogram()
```

```{r}
# (color = "white") outlines the geoms in white
ggplot(hikes, aes(x = elevation)) +
  geom_histogram(color = "white") 
```

```{r}
# (fill = "blue") fills geoms in with blue
ggplot(hikes, aes(x = elevation)) +
  geom_histogram(color = "white", fill = "blue") 
```

```{r}
# labs(x = "Elevation (feet)", y = "Number of hikes") labels the axes
ggplot(hikes, aes(x = elevation)) +
  geom_histogram(color = "white") +
  labs(x = "Elevation (feet)", y = "Number of hikes")
```
...
```{r}
# (bindwidth = 1000) sets the interval for the geoms as 1000 ft.
ggplot(hikes, aes(x = elevation)) +
...
```{r}
# (bindwidth = 5) sets the interval for the geoms as 5
ggplot(hikes, aes(x = elevation)) +
...
```{r}
# (bondwidth = 200) sets the interval for the geoms as 20 ft
ggplot(hikes, aes(x = elevation)) +
...
-   What's the difference between `color` and `fill`?
color defines the outline of the geom and fill defines the color of the geom

-   Why does adding `color = "white"` improve the visualization?
it clarifies the borders of each geom

-   What did `binwidth` do?
it defines the interval of the geoms

-   Why does the histogram become ineffective if the `binwidth` is too big (e.g. 1000 feet)?
it makes the data too general to see the variability, range, or outliers.

-   Why does the histogram become ineffective if the `binwidth` is too small (e.g. 5 feet)?
it does not generalize the data enough so you cannot see the typical outcome or shape of the data.
...
"color" will change the color of the line and "fill" will change the color of the area under the line.
...
It tells you the typical elevation through the highest points of the line, and it tells you the range by showing how much the line moves up and down. The shape and distribution of elevations is more apparent in this graph than the histogram.
...
Histogram pros:
- shows shape of data well

Histogram cons:
- less precise (doesn't show
individual data points)

Density plot pros:
- shows range and variability well
- shows outliers well

Density plot cons:
- doesn't show values for frequency of the x-axis data
