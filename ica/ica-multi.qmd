...
The sat scores range from 850 to 1100. The most frequent score is 900 and there is a smaller spike in scores around 1400.
...
The relationship between SAT scores and spending is negative for both salary and expenses on pupils. I would expect higher spending to yield higher SAT scores, so this result is surprising. However, the negative relationship is not very strong, especially for expenses.
...
```{r}
ggplot(education, aes(x = expend, y = sat, size = salary)) + geom_point() 
```
...
There is a strong negative relationship between both salary and scores and expenses and scores, on the lower end of the salary and expenses range. When salary and expenses get higher, their relationship with scores becomes weaker.
...
ggplot(education, aes(x = fracCat, y = sat)) + geom_violin()
```
...
```{r}
ggplot(education, aes(x = expend, y = sat, size = fracCat)) + geom_point()
```
...
It appeared that SAT scores decreased as spending increased because we were not taking into account the sample size of each data point, which means that every state's average was treated equally. But the highest scores actually came from the states in which a very small percentage of the students who took the SAT, and when you look at the trend among the states in which the majority of the students took the exam, the relationship with expenses is positive.
...
The most obvious outliers are Utah and California with respect to the 'ratio' metric.
